...
def percent_decode(value):
    """Decodes percent-encoded octets used by the PURL spec test corpus."""
    ...
    for _ in range(len(value)):
        ...
        if i + 2 >= len(value):
            return None, "Percent escape is incomplete"

        escape = value[i:i + 3].upper()
        decoded = _decode.get(escape)
        if decoded == None:
            return None, "Unsupported percent escape {}".format(value[i:i + 3])
